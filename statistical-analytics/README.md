...
- `regression-visual-guide.html`: visual guide of real R regression models using generated plots.
...
5. Use `regression-visual-guide.html`, `data/visual_regression_guide.csv`, and `scripts/build_visual_regression_guide.R` when the project needs real model visuals for teaching.
6. Use `data/regression_catalog.csv` when the project is specifically model-family selection.
7. Use `data/diagnostics_checklist.csv` during pre-model review.
8. Use `data/llm_statistical_analysis_checklist.csv` when the project evaluates LLM outputs, prompts, factuality, calibration, or drift.
9. Use `scripts/export_r_builtin_datasets.R` in RStudio to refresh sample CSVs.
10. Use `scripts/statistical_methods_cookbook.R`, `scripts/statistical_methods_cookbook.py`, and `sql/statistical_methods_cookbook.sql` as the cross-language technique codebooks.
11. Use `scripts/diagnostics_helpers.R` inside project notebooks and Quarto reports.
12. Use `scripts/download_open_data.R` or `scripts/download_open_data.py` when a lesson needs current public API data.
13. Use `sql/analytics_examples.sql` when building a DuckDB or database-backed workflow.
...
Then open `http://127.0.0.1:8123/statistical-analytics/index.html`, `http://127.0.0.1:8123/statistical-analytics/regression-visual-guide.html`, `http://127.0.0.1:8123/statistical-analytics/samples.html`, or `http://127.0.0.1:8123/statistical-analytics/model-tuning.html` from the repository root.
